...
class Datatables
{
  ...
  public function limit($lenght, $offset = 0)
  {
    $this->ci->db->limit($lenght, $offset);
  }
    ...
    if ($sSearch != '') {
      ...
      for ($i = 0; $i < count($mColArray); $i++) {
        ...
        if ($mColArray[$i]['searchable'] == 'true') {
          if ($this->check_cType()) {
            $sWhere .= "lower(CAST(" . $this->select[$mColArray[$i]['data']] . " AS TEXT)) LIKE lower('%" . $sSearch . "%') OR ";
          } else {
            //echo count($mColArray)."-".count($this->columns)."<br>";
            if (isset($this->columns[$i])):
              $sWhere .= "lower(CAST(" . $this->select[$this->columns[$i]] . " AS TEXT)) LIKE lower('%" . $sSearch . "%') OR ";
            endif;
          }
        }
      }
  ...
  private function produce_output($output, $charset)
  {
    ...
    foreach ($rResult->result_array() as $row_key => $row_val) {
      ...
      foreach ($this->edit_columns as $modkey => $modval) foreach ($modval as $val)
